...
} from './utils/auth-rate-limit.util';
...
export class AuthService {
  ...
  async register(
    ...
    await this.redisRateCheckLimit(
      ...
      {
        stage1Limit: this.configService.getOrThrow<number>(
          'RATE_REGISTER_STAGE1_LIMIT',
        ),
        stage1Time: this.configService.getOrThrow<number>(
          'RATE_REGISTER_STAGE1_TIME',
        ),
        stage2Limit: this.configService.getOrThrow<number>(
          'RATE_REGISTER_STAGE2_LIMIT',
        ),
        stage2Time: this.configService.getOrThrow<number>(
          'RATE_REGISTER_STAGE2_TIME',
        ),
      },
    );
    ...
    if (referralCode) {
      ...
      const referrer = await this.memberModel
        .findOne({ referralCode: normalizedReferralCode })
        .exec();
    ...
    const memberReferralCode = await this.generateUniqueReferralCode(
      dto.fullName,
    );
  ...
  async verifyOtp(
    dto: VerifyOtpDto,
    ipAddress: string,
  ): Promise<{ message: string; setupPasswordToken?: string }> {
    await this.redisRateCheckLimit(
      dto.verificationId,
      ipAddress,
      ...
      {
        stage1Limit: this.configService.getOrThrow<number>(
          'RATE_V_OTP_STAGE1_LIMIT',
        ),
        stage1Time: this.configService.getOrThrow<number>(
          'RATE_V_OTP_STAGE1_TIME',
        ),
        stage2Limit: this.configService.getOrThrow<number>(
          'RATE_V_OTP_STAGE2_LIMIT',
        ),
        stage2Time: this.configService.getOrThrow<number>(
          'RATE_V_OTP_STAGE2_TIME',
        ),
      },
    );
  ...
  async resendOtp(
    ...
    await this.redisRateCheckLimit(
      ...
      {
        stage1Limit: this.configService.getOrThrow<number>(
          'RATE_R_OTP_STAGE1_LIMIT',
        ),
        stage1Time: this.configService.getOrThrow<number>(
          'RATE_R_OTP_STAGE1_TIME',
        ),
        stage2Limit: this.configService.getOrThrow<number>(
          'RATE_R_OTP_STAGE2_LIMIT',
        ),
        stage2Time: this.configService.getOrThrow<number>(
          'RATE_R_OTP_STAGE2_TIME',
        ),
      },
    );
  ...
  async login(
    ...
    await this.redisRateCheckLimit(
      ...
      {
        stage1Limit: this.configService.getOrThrow<number>(
          'RATE_LOGIN_STAGE1_LIMIT',
        ),
        stage1Time: this.configService.getOrThrow<number>(
          'RATE_LOGIN_STAGE1_TIME',
        ),
        stage2Limit: this.configService.getOrThrow<number>(
          'RATE_LOGIN_STAGE2_LIMIT',
        ),
        stage2Time: this.configService.getOrThrow<number>(
          'RATE_LOGIN_STAGE2_TIME',
        ),
      },
    );
  ...
  async forgotPassword(
    ...
    await this.redisRateCheckLimit(
      ...
      {
        stage1Limit: this.configService.getOrThrow<number>(
          'RATE_FORGET_STAGE1_LIMIT',
        ),
        stage1Time: this.configService.getOrThrow<number>(
          'RATE_FORGET_STAGE1_TIME',
        ),
        stage2Limit: this.configService.getOrThrow<number>(
          'RATE_FORGET_STAGE2_LIMIT',
        ),
        stage2Time: this.configService.getOrThrow<number>(
          'RATE_FORGET_STAGE2_TIME',
        ),
      },
    );
  ...
  private async redisRateCheckLimit(
    ...
    if (!comboResult.allowed) {
      throwRateLimitException(
        `Too many requests. Please try again in ${comboResult.ttlSeconds} seconds.`,
      );
    }
    ...
    if (!ipResult.allowed) {
      throwRateLimitException(
        `Too many attempts from this network. Please try again in ${ipResult.ttlSeconds} seconds.`,
      );
    }
  }
}
