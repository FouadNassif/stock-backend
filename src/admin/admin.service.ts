...
export class AdminService implements OnModuleInit {
  ...
  async createAdmin(currentAdminId: string, dto: CreateAdminDto) {
    ...
    const existingMemberEmail = await this.memberModel
      .findOne({ email: normalizedEmail })
      .exec();
    ...
    try {
      await session.withTransaction(async () => {
        ...
        const adjustmentAmount =
          dto.type === WalletAdjustmentType.Credit ? dto.amount : -dto.amount;

        const balanceAfter = balanceBefore + adjustmentAmount;

        if (balanceAfter < 0) {
          throw new BadRequestException(
            'Wallet adjustment cannot make member balance negative',
          );
        }
