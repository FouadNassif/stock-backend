...
export class WalletService {
  ...
  async withdraw(
    ...
    const availableBalance =
      eligibleMember.walletBalance - pendingWithdrawalAmount;

    if (availableBalance < dto.amount) {
      throw new BadRequestException(
        `Insufficient available balance. Current balance is ${eligibleMember.walletBalance}, pending withdrawals total is ${pendingWithdrawalAmount}, available balance is ${availableBalance}`,
      );
    }

    if (!eligibleMember.lastDepositAt) {
      throw new BadRequestException(
        'Withdrawal is not available because no deposit was found',
      );
    }
    ...
    if (new Date() < withdrawAvailableAt) {
      throw new BadRequestException(
        `Withdrawal is allowed only after ${this.withdrawalHoldHours} hours from the most recent deposit`,
      );
    }
